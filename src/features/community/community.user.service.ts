...
export const getLobbyDashboard = async (userId: string) => {
  ...
  const { data: topGroups } = await supabase
    .from('study_groups')
    .select('id, name, group_level, total_xp')
...
export const startFocusSession = async (userId: string, topic: string) => {
  ...
  if (!profile?.current_group_id) {
    throw new Error("You must be in a study group to start a focus session");
  }
...
export const initiateBattle = async (userId: string) => {
  ...
  if (!profile?.current_group_id) {
    throw new Error("Join a group first to challenge others!");
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      initiated_by: userId,
      group_id: profile.current_group_id, // FIXED: Added group_id reference
      status: 'pending' 
    })
    .select()
    .single();
...
export const sendBuzz = async (senderId: string, targetUserId: string) => {
  const { error } = await supabase
    ...
    .insert({
      ...
      title_en: 'Buzz! 🔔', // FIXED: Corrected column name
      body_en: 'Your teammate just buzzed you to join the lobby/study session!', // FIXED: Corrected column name
      type: 'buzz',
      channel: 'in_app'
    });
