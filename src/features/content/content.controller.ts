...
export const createQuestion = catchAsync(async (req: AuthReq, res: Response) => {
  ...
  const payload: QuestionPayload = {
    ...
    comprehension_id: body.comprehension_id ? String(body.comprehension_id) : undefined,
    source_type: body.source_type ? String(body.source_type) : undefined,
...
export const getQuestionsForAudit = catchAsync(async (req: Request, res: Response) => {
  const filters: AuditFilterParams = {
    status: sanitizeQueryString(req.query.status),
    difficulty: sanitizeQueryString(req.query.difficulty),
    subject_id: sanitizeQueryString(req.query.subject_id),
...
export const getQuestionsBank = catchAsync(async (req: Request, res: Response) => {
  ...
  const filters: QuestionBankFilters = {
    subject_id: sanitizeQueryString(req.query.subject_id),
    chapter_id: sanitizeQueryString(req.query.chapter_id),
    topic_id: sanitizeQueryString(req.query.topic_id),
    difficulty: sanitizeQueryString(req.query.difficulty),
    type: sanitizeQueryString(req.query.type),
    status: sanitizeQueryString(req.query.status),
    search: sanitizeQueryString(req.query.search),
  };
